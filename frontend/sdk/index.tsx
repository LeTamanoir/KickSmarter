...
import { _assertWalletConnection, _getMethods, _getProject, _getStorage } from './utils';
import { pushImageToIPFS, getMetadataFromIPFS, getImageFromIPFS, pushMetadataToIPFS } from './ipfs';
...
const KicksmarterProvider = ({ contractAddress, children }: { contractAddress: string; children: React.ReactNode }) => {
	...
	const postProject = async ({
		...
		await methods.post_project(cid_metadata, funding_due_date, milestones).send();
	};
	...
	const fundProject = async (project_id: number, amount: number): Promise<void> => {
		_assertWalletConnection(connected);
	...
	const claimMilestone = async (project_id: number, milestone_id: number): Promise<void> => {
		_assertWalletConnection(connected);
	...
	const abortProject = async (project_id: number, milestone_id: number): Promise<void> => {
		_assertWalletConnection(connected);
