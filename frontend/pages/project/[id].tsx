...
import { VStack, HStack, Text, Image, Stepper, useSteps, Step, StepIndicator, StepSeparator, StepTitle, StepDescription, StepStatus, Progress, StepIcon, StepNumber, Box, Divider, Card, Stack, CardBody, Heading, CardFooter, Button, Input } from '@chakra-ui/react';
...
const ProjectDesc = (): JSX.Element => {
    ...
    return (
        <>
            <VStack w="100%" mb="30px">
                <Navbar />
                <Text pt="30px" fontSize="3xl" fontWeight="bold">{metadata?.title}</Text>
                ...
                <HStack align="start" pt="50px">
                    ...
                    <Text fontSize="xl" color="blue.500" fontWeight="bold">{project.status === "FUNDING" ? "Funding" : "Project in progress"}</Text>
                        ...
                        <>
                            ...
                            <Text>{project.current_amount} / {project.target_amount} xtz</Text>
                            <HStack>
                                <Input w="40%" type="number" value={fundValue} onChange={(event) => setFundValue(event.target.value)} />
                                <Button bg="#7CB4C4" onClick={supportProject}>Support this project</Button>
                            </HStack>
                        </>}
                        <VStack align="start" spacing="20px" pt="5px">
                            <Text><b style={{ fontSize: '25px'}}>{project.funding_due_date.toDateString()}</b><br /> end of funding</Text>
                            <Text><b style={{ fontSize: '25px' }}>{project.milestones[0].start_date.toDateString()}</b><br />next vote</Text>
                            <Text><b style={{ fontSize: '25px' }}>{project.votes.length}</b><br /> backers</Text>
                            {project.status === "IN_PROGRESS" && <>
                                <Button bg="#7CB4C4" onClick={disapproveMileston}>Disaprove milestone</Button>
                            </>}
                        </VStack>
                    </VStack>
                </HStack>
